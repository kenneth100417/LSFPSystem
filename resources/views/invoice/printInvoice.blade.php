    ...
    <div class="mt-4 mb-2" >
        ...
        <div class="row mt-2" >
            <div style="border-bottom: 1px solid #000000">
                <div class="mx-2">
                    <p class="text-dark" style="font-weight: bold; ">Sold to: <span style="font-weight: 0">{{$invoice == null ? '':$invoiceUser->firstname." ".$invoiceUser->lastname}}</span></p>
                    ...
                    <p class="text-dark" style="font-weight: bold; margin-top: -10px;">Order ID: <span style="font-weight: 0">{{$invoice == null ? '':"LSWP_ORDR".$invoice->id}}</span></p>
                </div>
            </div>
        <div>
    </div>
                    ...
                    <td style="width: 75px;">
                        <div>
                            <div class="d-flex py-1">
                                <img src="{{public_path('/uploads/products/'.$item->product->image)}}" class="avatar avatar-lg  me-3 border-radius-lg" alt="Shopping item" style="object-fit: cover; width:70px; height:70px; border-radius: 10px">
                            </div>
                        </div>
                    </td>
                    <td style="">
                        <div class="d-flex flex-column justify-content-center" >
                            <h6 class="mb-0 text-sm mt-2">{{$item->product->name}}</h6>
                            <div class="d-flex justify-content-between">
                                <div>
                                    <p class="text-xs text-dark mb-0">P {{number_format($item->price,2)}}</p>
                                </div>
                            </div>
                            <div class="text-end mt-1">
                                <p class="text-xs text-dark mb-0">x{{$item->quantity}}</p>
                                <p class="text-xs text-dark mb-0">P {{number_format($item->price * $item->quantity,2)}}</p>
                            </div>
                        </div>
                    </td>
                </tr> 
